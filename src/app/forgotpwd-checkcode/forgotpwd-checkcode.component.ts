...
export class ForgotpwdCheckcodeComponent implements OnInit {
  ...
  submit() {
    // if (this.passwordType.nativeElement.value.length < 6 || this.passwordType.nativeElement.value.length>13) {
    //   this.showPopFun("请输入6-13位密码");
    //   return;
    // }
    ...
    this.http.post('/api/m/consumer/findPwd.do?'+ this.signService.getStrUrl(data), AppConfig.httpOptions).subscribe(response => {
